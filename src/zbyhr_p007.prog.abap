...
*        city  LIKE pa0006-ort01,                            "Adres 1
        citytx         LIKE pa0006-ort01,
        "Adres 1
        district       LIKE pa0006-ort02,
        "Adres 2
        adress         LIKE iclcarerep1-satz,
        "Adres 3
        adressulke     LIKE hrp1000-stext,
        vergiulke      LIKE hrp1000-stext,
        phonegsm       LIKE pa0105-usrid,
        email          LIKE pa0105-usrid_long,
        iban           LIKE pa0009-iban,
        isfaizsiz      LIKE hrp1000-short,
        "Sertifika Fonu Faizsiz=T Aksi=F
        meslek         LIKE hrp1000-stext,
        ogrenimdurum   LIKE hrp1000-stext,
        anakizliksoyad LIKE p0002-nachn,
        uyruk          LIKE hrp1000-stext,
*        damga LIKE bseg-wrbtr,
        digerulkekodu  LIKE hrp1000-stext,
        ...
        sskno	         LIKE hrp1000-stext,
        sskno1         LIKE hrp1000-stext,
        "adr11-ssfid, "t7trg01-sskno,"İşyeri Sicil No 7 karakter
        ...
        cocuk          LIKE bseg-peinh,   "çocuk sayısı
        cocuk18        LIKE bseg-peinh, "18 yaşından büyük çocuk sayısı
        bos1           LIKE hrp1000-stext,
        bos2           LIKE hrp1000-stext,
*        siunv LIKE t7trg01-banka,
*        nakil LIKE pa0769-uname, "Nakil Olup Olmadığı
...
SELECTION-SCREEN BEGIN OF BLOCK block3 WITH FRAME TITLE TEXT-t02.
  PARAMETERS: odegun  LIKE bseg-peinh DEFAULT '30',
              isfaiz  TYPE zbyhr_de006 DEFAULT 'T',
              uyruk   LIKE pa0002-natio DEFAULT 'TC',
              dogum   LIKE pa0000-begda DEFAULT '19720101',
*            damga LIKE bseg-wrbtr DEFAULT '25.30',
...
*
                       ...
                       'ANSVH/ATX/WTEXT/BUTXT/ZPERGRUPTX'
                       'NO_OUT'
                       'X',
...
FORM fill_header.
  ...
  ls_line-info = 'BES Personel Listesi'.
...
*                AND   plans NE '99999999'.
    READ TABLE gt_t7trg01 WITH KEY werks = p0001-werks
                                   ...
                                   BINARY SEARCH.
    IF sy-subrc EQ 0.
      MOVE: gt_t7trg01-city TO lt_wb-cityc,
...
*            gt_t7trg01-sskno TO lt_wb-sskno,
  ...
  LOOP AT lt_wb.
    ...
    gt_out-odemegun = odegun.
    gt_out-isfaizsiz = isfaiz.
    gt_out-uyruk = uyruk.
    gt_out-adressulke = 'TÜRKİYE'.
    gt_out-vergiulke = 'TÜRKİYE'.
...
*    rp_provide_from_last p0021 '8   ' lt_wb-begda lt_wb-endda.
...
*                        p0009 TO gt_out.
    ...
    MOVE: p0006-ort02  TO gt_out-district,  " İlçe
          p0006-ort01  TO gt_out-citytx.  " İl

    gt_out-adress = p0006-stras.
    IF p0006-hsnmr IS NOT INITIAL.
      CONCATENATE gt_out-adress '/'
                  p0006-hsnmr
             INTO gt_out-adress.       " Adres alanlarını birleştir
    ENDIF.

    IF p0006-posta IS NOT INITIAL.
      CONCATENATE gt_out-adress '/'
                  p0006-posta
             INTO gt_out-adress.       " Adres alanlarını birleştir
    ENDIF.
    ...
    CONCATENATE gt_out-adress
                p0006-locat
           INTO gt_out-adress SEPARATED BY space. " Adres alanlarını
    ...
    CLEAR p0105.
    rp_provide_from_last p0105 'CELL' pn-begda pn-endda.
    IF sy-subrc EQ 0.
      gt_out-phonegsm = p0105-usrid.
    ENDIF.
    ...
    IF p0105-usrid_long IS INITIAL.
      ...
      IF p0105-usrid_long IS NOT INITIAL.
        gt_out-email = p0105-usrid_long.
      ENDIF.
    ELSE.
      gt_out-email = p0105-usrid_long.
    ENDIF.

    PERFORM read_egitim_info .

    LOOP AT p0021 WHERE subty EQ '2   '.
      ADD 1 TO gt_out-cocuk.
      IF p0021-fgbdt LT gv_fgbdt.
        ADD 1 TO gt_out-cocuk18.
      ENDIF.
    ENDLOOP.

    MOVE: "lt_wb-cityc TO gt_out-city,
...
*  ENDIF.
  ...
  ENDIF.
  IF dist NE space.
    CLEAR gt_out-district.
  ENDIF.
  IF adress NE space.
    CLEAR gt_out-adress.
  ENDIF.
  IF faizsiz NE space.
    CLEAR gt_out-isfaizsiz.
  ENDIF.
  IF meslek NE space.
*    CLEAR gt_out-meslek.
    gt_out-meslek = 'Diğer'.
  ENDIF.
  IF ogrenim NE space.
*    CLEAR gt_out-ogrenimdurum.
    gt_out-ogrenimdurum = 'Diğer'.
  ENDIF.
  IF anakizso NE space.
    CLEAR gt_out-anakizliksoyad.
  ENDIF.
  IF cocuk NE space.
    CLEAR gt_out-cocuk.
  ENDIF.
  IF cocuk18 NE space.
    CLEAR gt_out-cocuk18.
  ENDIF.
...
*educationalStatus    :     Eğitim durumu.  Gönderilebilecek değerler :

*           Unknown     :  Bilinmiyor
*           PrimarySchool   :  İlkOkul mezunu
*           SecondarySchool   :  OrtaOkul mezunu
*           HighSchool    :  Lise mezunu
*           VocationalHighSchool  :  Meslek Lisesi mezunu
*           AssociateDegree   : Ön Lisans Mezunu
*           Graduate      : Lisans Mezunu
...
*15	Lise
*20	Meslek Lisesi
*21	Yüksek tic.lisesi
*22	Meslek lisesi(san.)
*23	Meslek lisesi(tic.)
*25	Özel Lise
*26	Higher tect.college
*27	Higher sec.school
*30	Kız-Teknik-Olgunlaşm
*35	Yabancı Özel Lise
*40	Anadolu Lisesi
*45	Teknik Lise
*50	İmam Hatip Lisesi
*51	College
*52	Meslek yüksek okulu
*55	Fen Lisesi
*60	Ticaret Lisesi
*70	Açık Öğretim
*71	Harici kurs/seminer
*75	Ön Lisans
*80	Lisans
*85	Yüksek Lisans
*90	Executive MBA
*95	Doktora
    IF p0022-slart EQ '02' OR
       p0022-slart EQ '03' OR
       p0022-slart EQ '10'.
      gt_out-ogrenimdurum = 'İlk Okul'.
    ELSEIF p0022-slart EQ '05'.
      gt_out-ogrenimdurum = 'Orta Okul'.
    ELSEIF p0022-slart EQ '11' OR
           p0022-slart EQ '15' OR
           p0022-slart EQ '25' OR
           p0022-slart EQ '26' OR
           p0022-slart EQ '27' OR
           p0022-slart EQ '35' OR
           p0022-slart EQ '40' OR
           p0022-slart EQ '51' OR
           p0022-slart EQ '55' .
      gt_out-ogrenimdurum = 'Lise'.
    ELSEIF p0022-slart EQ '20' OR
           p0022-slart EQ '21' OR
           p0022-slart EQ '22' OR
           p0022-slart EQ '23' OR
           p0022-slart EQ '26' OR
           p0022-slart EQ '27' OR
           p0022-slart EQ '30' OR
           p0022-slart EQ '45' OR
           p0022-slart EQ '50' OR
           p0022-slart EQ '60' .
      gt_out-ogrenimdurum = 'Meslek Lisesi'.
    ELSEIF p0022-slart EQ '52' OR
           p0022-slart EQ '75'.
      gt_out-ogrenimdurum = 'Ön Lisans'.
    ELSEIF p0022-slart EQ '70' OR
           p0022-slart EQ '80' OR
           p0022-slart EQ ''.
      gt_out-ogrenimdurum = 'Lisans'.
    ELSEIF p0022-slart EQ '85'.
      gt_out-ogrenimdurum = 'Yüksek Lisans'.
    ELSEIF p0022-slart EQ space.
      gt_out-ogrenimdurum = 'Diğer'.
    ENDIF.
