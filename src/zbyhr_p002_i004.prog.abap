...
*   Gerekli olan dataların çekilmesi
  ...
  SORT gt004 ASCENDING BY anagr altgr .
  SORT gt005 ASCENDING BY slga ddntk.
  SORT gt006 ASCENDING BY lgart .
...
FORM last_process .
  ...
  SORT w ASCENDING BY val01 val02 anagr altgr.
...
FORM find_sub_total .
  ...
  LOOP AT gt_kostl.
    ...
    LOOP AT total
      ...
      CASE total-anagr.
        WHEN '01'. " Normal İstihkak, Fazla Mesai
          sub_t-brut = sub_t-brut + total-amt.
        WHEN '02'. " Ek Ödeme
          sub_t-brut = sub_t-brut + total-amt.
        WHEN '03'. " Kesintiler Toplami
          sub_t-kesg = sub_t-kesg + total-amt.
        WHEN '04'.
        WHEN '05'. " Özel Kesintiler Toplami
          IF total-altgr EQ '01'.
            sub_t-kesg = sub_t-kesg + total-amt.
          ENDIF.

        WHEN '06'."Toplam Maliyet
          sub_t-tmal = sub_t-tmal + total-amt."
      ENDCASE.
    ENDLOOP.
*    LOOP AT total WHERE val01 EQ gt_kostl-val01
*                  AND val02 EQ gt_kostl-val02
*                  AND anagr EQ '04'
*                  AND altgr EQ '02'.
*     sub_t-tmalt = sub_t-tmalt - total-amt.
*    ENDLOOP.
*    sub_t-tmalt = sub_t-tmalt + sub_t-tmal.
*    sub_t-neto  = sub_t-brut  - sub_t-kesg .

    LOOP AT w  WHERE val01 EQ gt_kostl-val01
                 AND val02 EQ gt_kostl-val02
                 AND anagr EQ '04'
                 AND altgr EQ '02'
                 AND slga NE '/DIS'
                 AND seqno NE '250'.
      sub_t-tmalt = sub_t-tmalt - w-amt.
    ENDLOOP.
    sub_t-tmalt = sub_t-tmalt + sub_t-tmal.
    sub_t-neto  = sub_t-brut  - sub_t-kesg .
    CLEAR w.
...
*    params-pdest = 'ZPDF'.
      ...
      SUBMIT zbyhr_p002 WITH SELECTION-TABLE i_rsparams
                              TO SAP-SPOOL
